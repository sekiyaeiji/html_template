...
- img要素の表示サイズは可能な限りwidth属性、height属性によって指定する
...
|種別|規則|
|:--|:--|
|CSS|`[category]-[機能・用途（省略可）].css`  【例】`about.css`|
|JavaScript|`[category]-[機能・用途（省略可）].js`  【例】`common.js`|
...
- 写真、サムネイル、挿絵などを除くUIパーツとしての画像表現は可能な限りCSSのbackground-imageによって実装する
- img要素の表示サイズは可能な限りhtmlのwidth属性、height属性によって指定し、CSSによる指定は必要最低限にとどめる
...
|バナー|/stat/img/banner/|
|コンテンツ固有モジュール別単体ファイル|/stat/img/module/[category]|
|その他|/stat/img/|
